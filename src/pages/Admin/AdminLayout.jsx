...
const AdminLayout = () => {
  ...
  // Path change hone par mobile menu close ho jaye
  useEffect(() => {
  ...
  return (
    <div className="flex min-h-screen bg-[#F4F7FE] font-sans selection:bg-indigo-500/10">
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-4 px-5 py-4 rounded-2xl transition-all relative group ${
                  isActive ? "bg-slate-950 text-white shadow-2xl shadow-slate-400" : "text-slate-400 hover:bg-slate-50 hover:text-slate-900"
                }`}
              >
                <span className={isActive ? "text-indigo-400" : "group-hover:text-indigo-600"}>{item.icon}</span>
                <span className="text-sm font-black uppercase tracking-tight">{item.label}</span>
                {isActive && <div className="ml-auto size-2 bg-indigo-400 rounded-full animate-pulse shadow-[0_0_8px_rgba(129,140,248,0.8)]" />}
              </button>
            );
          })}
        </nav>
      ...
      {/* 2. MOBILE DRAWER (Sidebar for mobile) */}
      <AnimatePresence>
        {isMobileMenuOpen && (
          <>
            {/* Overlay */}
            ...
            >
              <div className="flex justify-between items-center mb-10">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-lg font-black uppercase">Admin Pro</span>
                </div>
                ...
                {navItems.map((item) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl font-black uppercase tracking-widest text-[11px] transition-all ${
                      location.pathname === item.path ? "bg-slate-950 text-white shadow-lg" : "text-slate-400"
                    }`}
                  >
              ...
              <button onClick={() => navigate("/")} className="mt-auto w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-red-500 font-black text-[11px] uppercase tracking-widest bg-red-50">
        ...
        <header className="h-20 lg:h-24 bg-[#F4F7FE]/80 backdrop-blur-xl sticky top-0 z-40 px-4 lg:px-10 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:block">
              ...
              <h2 className="text-xl lg:text-2xl font-black text-slate-900 tracking-tight uppercase tracking-tighter leading-none">Intelligence Hub</h2>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3 bg-white p-1.5 sm:p-2 rounded-[24px] shadow-sm border border-slate-100">
             ...
             <div className="hidden md:flex items-center gap-3 bg-[#F4F7FE] px-5 py-2.5 rounded-2xl border border-slate-100">
                ...
                <input type="text" placeholder="Search..." className="bg-transparent border-none outline-none text-[11px] font-bold text-slate-700 w-32" />
             </div>
